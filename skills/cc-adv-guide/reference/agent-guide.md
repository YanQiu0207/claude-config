...
| 字段 | 类型 | 必需 | 默认值 | 说明 |
|------|------|------|--------|------|
| `tools` | `string`（逗号分隔）或 `string[]` | 否 | 继承所有工具 | 允许使用的工具白名单。支持 `Agent(worker, researcher)` 语法限制可派生的 subagent 类型（**仅在主线程模式下生效**，即通过 `claude --agent` 启动时；作为 subagent 运行时此语法无效） |
...
**此语法仅在主线程模式下生效**（通过 `claude --agent` 启动时）。作为 subagent 运行时，`Agent()` 无效——subagent 不能派生其他 subagent。

```yaml
# 只能派生 worker 和 researcher 两种 subagent（需以 --agent 启动）
...
**要点**：`Agent(worker, researcher)` 语法限制只能派生这两种 subagent，防止无限制的 agent 调用链。此语法仅在主线程模式（`claude --agent coordinator`）下生效；若 coordinator 作为 subagent 被派生，则无法再派生其他 subagent。
...
- **subagent 不能嵌套派生其他 subagent**：通过 Agent 工具或 @mention 派生的 subagent 内部无法调用 Agent 工具。`Agent()` 语法写在 subagent 定义的 `tools` 中无效。需要嵌套委托时，用 Skill 或从主对话链式调用 subagent。注意：通过 `claude --agent` 启动的 agent 是**主线程**，不受此限制
