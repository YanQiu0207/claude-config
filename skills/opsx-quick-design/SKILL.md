---
name: opsx-quick-design
description: OpenSpec 轻量级设计工作流，适用于内部工具、小型服务等不需要完整 spec 流程的场景。AI 可主动提出设计方案，产出 Quick Draft 的 proposal.md。
---

Using opsx-quick-design

# OpenSpec 快速设计

## 适用场景 vs 不适用场景

| 适用 | 不适用（请用 `/opsx-system-design`） |
|------|--------------------------------------|
| 内部工具（管理后台、运维脚本、异常管理服务） | 核心业务系统的新功能 |
| 小型独立服务（< 3 个核心模块） | 涉及多团队协作的接口设计 |
| 开发者即是设计者、需求相对清晰 | 高风险改动（现有系统的架构变更） |
| 需要快速出方案、不想走完整 spec 流程 | 需要严格 sign-off 的正式设计评审 |

## AI 角色

快速设计中，AI **主动承担设计工作**，而非仅扮演引导者：
- 读完代码背景后直接提出架构方案
- 识别关键风险和权衡，主动说明
- 用户的角色是评审和决策，而非从零描述设计

---

## 工作流程

### Step 1：收集 Brief（1-2 轮）

向用户询问以下信息（一次问完，不分轮）：

```
请简要描述：
1. 这个工具/服务要解决什么问题？
2. 主要用户是谁（运维？开发？内部系统自动触发）？
3. 有什么关键约束？（性能要求、依赖限制、截止时间等）
4. 是全新项目，还是在已有系统上扩展？
```

用户回答后，如有明显信息缺口，追问一轮；信息足够则直接进入 Step 2。

### Step 2：代码调研（新系统可跳过）

如果是在已有系统上扩展，AI **自主**调研相关代码：
- 读取相关模块结构、接口、依赖关系
- 记录影响设计的技术约束

调研完成后，用一段简短摘要（3-5 条 bullet）告知用户，然后进入 Step 3。

**不要**问用户「现有实现是什么样的」，这些 AI 应该自己读代码获取。

### Step 3：AI 提出设计方案

直接给出一个具体的设计方案，包含：

```markdown
**整体方案**
[一句话描述架构思路]

**核心组件**
- [组件 1]：[职责]
- [组件 2]：[职责]

**主要接口/API**（如适用）
- [接口描述]

**数据流**（如复杂）
[数据如何流转]

**关键权衡**
- [选择 A 的原因，以及放弃 B 的原因]
- [已知风险点]

**运维考量**
- 部署方式：[说明]
- 关键指标：[需要监控什么]

**验收标准**
- [怎样判断这个工具/服务可用]
```

然后问用户：「这个方向符合你的预期吗？有什么需要调整的？」

### Step 4：迭代确认（通常 1-2 轮）

根据用户反馈调整方案，直到用户确认「好的」「可以」「就这样」类似表述。

**如果用户对方案有重大异议**（不是细节调整，而是方向性不同），重新回到 Step 3。

### Step 5：写入 proposal.md

用户确认后，立即初始化变更目录并写入文件。

先加载 `opsx-project-knowledge` 确认目录路径与归档约定。

**确认变更名称**（如用户还未提供）：

```
这次变更怎么命名？格式建议：动词-名词（如 add-xxx-tool）
```

**初始化目录**：

```bash
mkdir -p openspec/changes/<change-name>/specs/
cp skills/opsx-requirements-clarification/reference/openspec_yaml_template.yaml \
   openspec/changes/<change-name>/.openspec.yaml
cp skills/opsx-quick-design/reference/quick-proposal-template.md \
   openspec/changes/<change-name>/proposal.md
```

在 `proposal.md` 中填写确认后的方案内容，在 `.openspec.yaml` 中将 `status` 标注为 `Quick Draft`。

写完后，告知用户文件路径；如果已包含问题、目标、核心方案、关键接口/数据模型（如适用）和验收标准，可作为 `/opsx-code-generation` 的输入。然后询问是否需要立即拆分任务（创建 tasks.md）。

### Step 6：任务规划（可选，当场完成）

如果用户同意，立即基于方案拆分任务并创建 `tasks.md`：
- 每个任务有明确的完成条件
- 按依赖顺序排列
- 预计复杂度（小/中/大）
- 如涉及新模块、接口变更或架构变更，加入「更新项目知识文档」任务

---

## 强制规则

1. **Brief 信息不足时不要开始设计**：缺少问题描述或关键约束时，先问清楚
2. **AI 先提方案**：不要反问「你有什么思路」，直接给方案让用户评审
3. **说明权衡**：每个设计决策要说明为什么选这个，放弃了什么
4. **写入用户确认的内容**：`proposal.md` 内容必须是用户已确认的方案，不要把探讨中的内容写进去
5. **不要写空章节**：`quick-proposal-template.md` 中未用到的章节直接省略

## 反模式

| ❌ 错误做法 | ✅ 正确做法 |
|------------|-----------|
| 问用户「你想怎么设计？」 | AI 直接提出方案，用户评审 |
| 把 5 个问题分 5 轮问 | 一次问完所有信息 |
| 方案描述过于抽象（「用微服务架构」） | 给出具体组件和接口 |
| 方案确认后不写文件 | 立即写入 `proposal.md` |
| 强迫用户填所有章节 | 只填有内容的章节 |
