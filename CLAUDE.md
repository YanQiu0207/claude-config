\# 全局指令

\## 沟通方式
\- 使用中文回复

\## 通用代码风格
\- 缩进使用 4 空格

\## Claude Code 参考知识库
\- 参考文件：`~/.claude/claude_ref/claude-code-guide.md`
\- **查询时**：回答 Claude Code 相关问题时
  \- 模型自身有明确答案：直接回答；若此文件中缺少该内容，询问用户是否更新到此文件
  \- 模型自身没有明确答案：查阅此文件作为补充；若参考了此文件内容，要告知用户信息来源
\- **记录时**：当用户要求"记录"某个知识点时
  \- Claude Code 相关内容 → 写入此文件
  \- 其他内容 → 先询问用户写入位置（某个具体文件、项目记忆、还是全局记忆）

\## 中文 Markdown 排版规范
\- 规范技能：`md-zh`（使用 Skill 工具调用以获取完整规则）

\## 记忆与知识写入规则

\- **行为规则的持久化位置**：
  \- 全局适用（所有项目都应遵守）→ 写入 `~/.claude/CLAUDE.md`
  \- 当前项目专属 → 写入项目记忆
  \- 默认优先考虑全局，除非规则明显只与某个项目相关
\- **写入 Claude Code 知识库前必须验证**：向 `claude-code-guide.md`、`cc-adv-guide` 等知识库写入 Claude Code 行为特性时（架构、限制、权限等），必须先 WebFetch 官方文档确认，不能凭推断写入

\## 创建技能的规则
\- 创建新的文件处理类技能时，主动询问用户是否需要支持批量（多文件）执行
\- 如果需要批量支持，先用 Skill 工具调用 `cc-adv-guide`，按照其中的三层架构设计模式来实现
\- 回答 skills、agents 相关问题时，可用 Skill 工具调用 `cc-adv-guide` 对回答进行补充，非优先使用这个技能
