...
Claude Code 的全局行为配置，定义沟通语言（中文）、事实性内容必须有来源支撑的要求、代码风格（4 空格缩进）、Claude Code 参考知识库路径、中文 Markdown 排版规范路径（通过 `md-zh` 技能）、记忆与知识写入规则（全局 vs. 项目的持久化位置）、创建或修改技能时先调用 `cc-adv-guide` 获取设计规范等。安装时注意不要覆盖本地已有的 `CLAUDE.md`，应手动合并。
